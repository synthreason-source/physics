...
FIXES over original:
  1. engine_integral: inner sample loop was range(0) — fixed to sample
     SAMPLES_PER_TICK points every tick, giving real Monte-Carlo convergence.
  2. engine_roots: annealer now detects convergence (residual < 1e-8) and
     exits early instead of always running to MAX_CYCLES.
  3. engine_roots: explores BOTH roots by running two independent chains
     (warm/cold restart) so both solutions are actually found by annealing.
  4. engine_factor Fermat phase: a_new lower-bound clamped correctly;
     energy display formula normalised so it doesn't overflow the bar.
  5. All engines: disp_e clamped to [0, 100] consistently.
"""
...
MAX_CYCLES       = 44000
TICK_MS          = 1
SAMPLES_PER_TICK = 40   # Monte-Carlo samples per generator tick (was 0 — BUG FIX)
...
def engine_factor(n):
    ...
    Phase 1: Fermat's method — fast when p ≈ q.
    Phase 2: Trial-division sweep — catches unbalanced pairs.
    """
    ...
    while cycle < fermat_limit:
        ...
        def energy(av):
            b2 = av * av - n
            if b2 < 0:
                return min(100, (-b2) / max(n, 1) * 100)
            b  = int(b2 ** 0.5)
            gap = b2 - b * b
            return min(100, gap / max(av, 1) * 10)
        ...
        disp_e = energy(a)
        log = (f"[{cycle:3d}] T={T:.3f}  a={a}  b²={a*a-n}  E={disp_e:.1f}"
               ) if cycle % 15 == 0 else None
        yield disp_e, log, False, None, None

    yield 50, f"[{cycle}] Fermat stalled — switching to trial-division sweep.", False, None, None
        ...
        if n % pp == 0:
            ...
            yield 0, None, True, \
                  ...
                  (f"Trial-division sweep found factor {pp} at cycle {cycle}.\n"
                   f"q = N ÷ p = {q}.\n"
                   f"Size ratio {ratio:.4f} — {bal} pair.")
            return
        if cycle % 200 == 0:
            prog   = pp / max(root, 1)
            disp_e = min(100, prog * 80)
            yield disp_e, f"[{cycle:3d}] trial p={pp}  progress={prog*100:.1f}%", False, None, None
...
def engine_roots(a, b, c):
    """
    Quadratic root finder via simulated annealing.
    FIX: convergence detection added — exits as soon as residual < 1e-8.
    FIX: two independent chains find both roots separately.
    """
    disc = b * b - 4 * a * c

    # Run two annealing chains to find each root independently
    chains = [random.uniform(-15, 0), random.uniform(0, 15)]
    found  = []

    for start_x in chains:
        x     = start_x
        limit = MAX_CYCLES // 2
        for cycle in range(1, limit + 1):
            T     = max(0.005, 1.0 - cycle / limit)
            ...
            log    = (f"[chain{'AB'[len(found)]} {cycle:3d}] T={T:.3f}  "
                      f"x={x:.5f}  residual={cur_e:.2e}"
                      ) if cycle % 15 == 0 else None
            yield disp_e, log, False, None, None

            # Early exit when annealer has converged
            if cur_e < 1e-8:
                found.append(round(x, 8))
                break
        else:
            found.append(round(x, 8))   # best effort after limit

    # Final answer — use exact formula for display accuracy
    if disc < 0:
        re_  = -b / (2 * a)
        im_  = math.sqrt(-disc) / (2 * a)
        ans  = f"x = {re_:.8f} ± {im_:.8f}i  (complex pair)"
        ...
        ans  = f"x₁ = {r1:.8f}\nx₂ = {r2:.8f}"
        a1, a2 = sorted(found), sorted([r1, r2])
        sub  = (f"Δ = {disc:.6f} > 0 → two distinct real roots.\n"
                f"Annealer found ≈ {found[0]:.5f}, {found[1]:.5f}  "
                f"(exact: {r1:.5f}, {r2:.5f})")

    yield 0, f"Solution locked. disc={disc:.6f}", True, ans, sub
...
def engine_tsp(cities):
    ...
    def total(r):
        return sum(dist(cities[r[i]], cities[r[(i + 1) % n]]) for i in range(n))
    ...
    while cycle < MAX_CYCLES:
        ...
        log    = (f"[{cycle:3d}] T={T:.3f}  len={total(route):.3f}  best={best_d:.3f}"
                  ) if cycle % 15 == 0 else None
        yield disp_e, log, False, None, None
    ...
    yield 0, f"[{MAX_CYCLES}] Tour locked. Length={best_d:.4f}", True, \
          ...
          (f"{n}-city TSP via photonic 2-opt annealing.\n"
           f"Evaluated {MAX_CYCLES} swap configurations.")
...
def engine_integral(f_str, a, b):
    ...
    FIX: original inner loop was range(max(1, 200//MAX_CYCLES)) = range(0),
    meaning ZERO samples were ever taken.  Fixed to SAMPLES_PER_TICK per tick.
    """
    try:
    ...
    y_min   = min(samples) - 0.1
    y_max   = max(samples) + 0.1
    ...
    while cycle < MAX_CYCLES:
        ...
        # ── FIX: actually take SAMPLES_PER_TICK samples every tick ──
        ...
        log    = (f"[{cycle:3d}] samples={total_pts}  ∫≈{est:.6f}  σ∝{sigma:.4f}"
                  ) if cycle % 15 == 0 else None
        yield disp_e, log, False, None, None
    ...
    h    = (b - a) / 1000
    simp = f(a) + f(b) + sum(
        (4 if i % 2 else 2) * f(a + i * h) for i in range(1, 1000))
    simp *= h / 3

    yield 0, f"[{MAX_CYCLES}] Integration complete. ∫≈{est:.8f}", True, \
          (f"∫ {f_str} dx  from {a} to {b}\n"
           f"≈ {est:.8f}  (Monte Carlo, {total_pts:,} samples)\n"
           f"≈ {simp:.8f}  (Simpson cross-check)\n"
           f"Δ = {abs(est - simp):.2e}"), \
          (f"Monte Carlo: {total_pts:,} photonic beam samples.\n"
           f"Simpson 1000-segment cross-check confirms result.")
...
def make_problems():
    ...
    cities = [(round(random.uniform(0, 20), 1),
               round(random.uniform(0, 20), 1)) for _ in range(5)]
    ...
    return {
        ...
        f"Quadratic  {qa}x² + ({qb})x + ({qc}) = 0": {
            ...
            "display": (f"Solve  {qa}x² + ({qb})x + ({qc}) = 0\n"
                        ...
                        f"(two independent annealing chains, convergence-detected)"),
        ...
        f"5-city TSP  (random layout)": {
            ...
            "display": (f"Find shortest tour through 5 cities:\n"
                        f"{city_desc}\n"
                        f"(answer unknown — solved by 2-opt annealing)"),
        ...
        f"Dominant eigenvalue  3×3": {
            ...
            "display": (f"Dominant eigenvalue of random 3×3 matrix:\n"
                        ...
                        f"(power iteration via photonic beam)"),
        ...
        f"Integrate  {f_str}": {
            ...
            "display": (f"Compute  ∫ {f_str} dx\n"
                        ...
                        f"(photonic Monte Carlo — {SAMPLES_PER_TICK} samples/tick)"),
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self._log_text = tk.Text(lo, height=4, font=("Courier", 9),
    ...
    def _reset(self):
        if self._sim_job:
            self.after_cancel(self._sim_job); self._sim_job = None
        self._running = False; self._cycles = 0
        self._energy_hist = []; self._sim_gen = None
    ...
    def _tick(self):
        ...
        if done:
            self._finish(answer or "No solution", sub or "")
        else:
            self._sim_job = self.after(TICK_MS, self._tick)
