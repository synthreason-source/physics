...
struct Config {
    ...
    bool quiet        = false;
    // CSV input
    std::string csv_file;         // path to CSV; empty = random generation
    int         csv_col   = 0;    // 0-based column index (used when csv_col_name is empty)
    std::string csv_col_name;     // named column header (overrides csv_col when set)
    bool        csv_header = true;// first row is a header
    char        csv_delim  = ','; // field delimiter
};
...
"\n"
"CSV format notes:\n"
"  Rows with non-numeric or empty values in the chosen column are skipped.\n"
"  Duplicate values are deduplicated with a warning.\n"
"  Values are read as integers (fractional parts truncated).\n"
"  Up to 128 values are loaded; extras are truncated with a warning.\n"
"\n"
"Examples:\n"
"  " << prog << "\n"
"  " << prog << " --set-size 20 --seed 42\n"
"  " << prog << " --csv numbers.csv --target 1337\n"
"  " << prog << " --csv data.csv --csv-col-name amount --csv-delim ;\n"
"  " << prog << " --csv data.csv --csv-no-header --csv-col 2\n"
...
// CSV loader
//
// Reads a single column of integers from a delimited text file.
// - Supports any single-character delimiter (default ',').
// - Header row is optional; column can be identified by index or name.
// - Non-numeric and empty cells in the target column are skipped with a warning.
// - Duplicate values are deduplicated with a warning.
// - At most 128 values are loaded (extras warn + truncate).
// - Values are parsed as long long then range-checked to int.
// ============================================================================
static std::vector<int> load_csv(const Config& cfg) {
    std::ifstream f(cfg.csv_file);
    if (!f) {
        std::cerr << "Cannot open CSV file: " << cfg.csv_file << "\n";
        std::exit(1);
    }

    int col_idx = cfg.csv_col;   // resolved below if csv_col_name is set
    std::string line;
    int line_num = 0;
    ...
    if (cfg.csv_header) {
        if (!std::getline(f, line)) {
            std::cerr << "CSV file is empty: " << cfg.csv_file << "\n";
            std::exit(1);
        }
        ++line_num;
        ...
        if (!cfg.csv_col_name.empty()) {
            ...
            if (!found) {
                std::cerr << "CSV column '" << cfg.csv_col_name
                          << "' not found in header of " << cfg.csv_file << "\n";
                std::exit(1);
            }
        }
    }
    ...
    while (std::getline(f, line)) {
        ...
        while (std::getline(ss, cell, cfg.csv_delim)) {
            if (ci == col_idx) {
                ...
                // strip a trailing decimal part (e.g. "42.0" → "42")
                size_t dot = tok.find('.');
                ...
                if (*endp != '\0' && *endp != '\r') {
                    if (!cfg.quiet)
                        std::cerr << "  [CSV] line " << line_num
                                  << ": skipping non-numeric value '" << tok << "'\n";
                    ++skipped;
                    break;
                }
                if (v < INT_MIN || v > INT_MAX) {
                    if (!cfg.quiet)
                        std::cerr << "  [CSV] line " << line_num
                                  << ": value " << v << " out of int range, skipping\n";
                    ++skipped;
                    break;
                }
                int iv = (int)v;
                ...
                // dedup check
                if (std::find(numbers.begin(), numbers.end(), iv) != numbers.end()) {
                    if (!cfg.quiet)
                        std::cerr << "  [CSV] line " << line_num
                                  << ": duplicate value " << iv << ", skipping\n";
                    ++dupes;
                    break;
                }

                // 128-element cap
                if ((int)numbers.size() >= 128) {
                    ++truncated;
                    break;
                }
        ...
        if (!row_found && !cfg.quiet)
            std::cerr << "  [CSV] line " << line_num
                      << ": fewer than " << (col_idx + 1) << " columns, skipping\n";
    }

    if (!cfg.quiet) {
        std::cout << "[CSV] loaded " << numbers.size() << " value(s) from '"
                  << cfg.csv_file << "' (col ";
        if (!cfg.csv_col_name.empty()) std::cout << "'" << cfg.csv_col_name << "'";
        else                           std::cout << col_idx;
        std::cout << ")";
    ...
    if (numbers.empty()) {
        std::cerr << "No usable integers found in CSV column.\n";
        std::exit(1);
    }
...
static void run_demo(const Config& cfg) {
    ...
    if (target < 0) {
        int cap = std::min(actual_size - 1, 6);
    ...
    if (!cfg.quiet) {
        ...
        std::cout << "Config: set-size=" << actual_size
                  << "  seed=" << seed
                  << "  threads=" << cfg.threads
                  << "  blocks=" << cfg.blocks << "\n";
        std::cout << "Mask:   lo=bits[0-63]  hi=bits[64-127]  "
                  << (actual_size > 64 ? "WIDE kernel (n>64)" : "standard kernel (n<=64)")
                  << "\n\n";
