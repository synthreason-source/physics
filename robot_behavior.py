...
def run_auto_quantum_knapsack(items, target_capacity, shots=4000):
        ...
        if count > threshold:
            ...
            for i in range(total_q):
                if bitstring[total_q - 1 - i] == '1':
                    selected_labels.append(f"{labels[i]} time taken({weights[i]})")
    ...
    if not formatted_counts:
        ...
        # Grab the top 3 pieces of random noise so plot_histogram doesn't crash
        sorted_noise = sorted(raw_counts.items(), key=lambda x: x[1], reverse=True)[:3]
        for bitstring, count in sorted_noise:
            # Still format the noise so it looks like a valid label
            selected_labels = []
            for i in range(total_q):
                if bitstring[total_q - 1 - i] == '1':
                    selected_labels.append(f"{labels[i]} ({weights[i]})")
            label_text = "Noise: Empty" if not selected_labels else "Noise:" + "+".join(selected_labels)
            formatted_counts[label_text] = count
    else:
...
def make_auto_inventory(n_items=80, min_weight=1, max_weight=100):
    ...
    chosen_labels = [f"behavior_{i+1}" for i in range(n_items)]
...
def make_auto_inventory_offset(n_items=80, min_weight=1, max_weight=100):
    # Instead of a small pool of Greek letters, generate numbered strings 
    # so we never run out of unique labels, no matter how high n_items goes.
    chosen_labels = [f"target_offset_{i+1}" for i in range(n_items)]
...
my_inventory = make_auto_inventory(n_items=12)

my_inventory_offset = make_auto_inventory_offset(n_items=4)

my_inventory_combined = my_inventory | my_inventory_offset

# Plant a hidden combination to guarantee a valid target sum
hidden_keys = random.sample(list(my_inventory.keys()), 3)
hidden_keys.extend(random.sample(list(my_inventory_offset.keys()), 3))
target_weight = sum(my_inventory_combined[k] for k in hidden_keys) # or anything here

print(f"[AUTO-GEN] Planted a valid combination: {hidden_keys} which sums to {target_weight}")

# Run the simulation
run_auto_quantum_knapsack(my_inventory_combined, target_weight)